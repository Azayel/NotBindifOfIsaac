...
public class RedBooster extends AbstractAnimatedGameObject implements IInteractable {

    public RedBooster(double _x, double _y){

        super(_x, _y, 0, 0, Isaac_TextureItems.R_BOOSTER, true);
    }

    @Override
    public void interact(AbstractGameObject avatar) {
        if(avatar instanceof Isaac_Avatar isaacAvatar){
            ((Isaac_World)world).mod_damage();
